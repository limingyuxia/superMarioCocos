import {keyStateEnum, deepClone, labelData} from "../utils";
...
export class moveCtrl {
    ...
    constructor(target: mario, midPointX: number, tiledMap: tiledMap) {
        this.owner = target;
    ...
    public upData(dt: number): void {
        ...
        // 计算X方向
        let dirX = (keyState.get(keyStateEnum.RIGHT) ? 1 : 0) 
            + (keyState.get(keyStateEnum.LEFT) ? -1 : 0);
        ...
        //没有上升到最大高度时松开按键
        if (this.jumpKeyTime != 0 && !keyState.get(keyStateEnum.UP)) {
            this.jumpKeyTime = 0;
            this.isLand = true;
        }

        //长按跳跃键
        if (keyState.get(keyStateEnum.UP) && !this.isLand) {
            if (this.jumpKeyTime) {
        ...
        if (speed.y <= 0) {
            ...
            //下降过程Y方向碰撞检测
            if ( this.collisionLand(position, "land", cc.v2(0, -size.height / 2 + offset)) ||
                this.collisionLand(position, "block", cc.v2(0, -size.height / 2 + offset)) ||
                this.collisionLand(position, "pipe", cc.v2(0, -size.height / 2 + offset)) ) {
      
                if (!keyState.get(keyStateEnum.UP)) {
                    this.isLand = false;
                }
            ...
            //是否掉进陷进层
            if ( this.collisionLand(position, "trap", cc.v2(0, -size.height / 2 + offset)) ) {
                eventScokoban.emit("GameOver", true);
            }
        } else {

            //上升过程Y方向碰撞检测
            if (this.collisionLand(
                position,
                "block",
        ...
        if (tiledLayer.isCollision(tiled, "coin")) {
         ...
         //与敌人碰撞
        const nodes = tiledObject.getCollisionNodes(this.owner.node.getBoundingBox());
        const enemyComps = nodes.map((node) => node.getComponent("enemy")).filter((comp) => !!comp);

        if ((nodes.length > 0 && speed.y >= 0) || nodes.length !== enemyComps.length) {
    ...
    private upDataX(offsetX: number): void {
        ...
        if (posX <= size.width / 2 + this.leftBorder || posX >= this.rightBorder - size.width / 2)
            return;
    ...
    public collisionLand(
        ...
        // 是否与Land/block碰撞
        const tiled = tiledObject.getTiledByPos(marioPosition.add(offset));
    ...
    protected _collisionLand(
        ...
        if ( this.collisionLand(position, "block", cc.v2((dir * size.width) / 2, 0)) ||
            this.collisionLand(position, "block", cc.v2((dir * size.width) / 2, size.height / 2 + 5)) ||
            this.collisionLand(position, "block", cc.v2((dir * size.width) / 2, -size.height / 2 + 15)) ||
            this.collisionLand(position, "pipe", cc.v2((dir * size.width) / 2, 0)) ||
            this.collisionLand(position, "pipe", cc.v2((dir * size.width) / 2, size.height / 2 + 5)) ||
            this.collisionLand(position, "pipe", cc.v2((dir * size.width) / 2, -size.height / 2 + 15))
        ) {
            return true;
        }

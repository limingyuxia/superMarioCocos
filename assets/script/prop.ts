...
export default class prop extends cc.Component {
    ...
    update(dt: number) {
        ...
        //判断是否站立在地面
        if (this.collision(curPosition, "land", cc.v2(0, -size.height / 2 - 4)) ||
        this.collision(curPosition, "block", cc.v2(0, -size.height / 2 - 4))) {
            ...
            if ( this.collision(position, "block", cc.v2((size.width / 2) * this.dir, 0)) ||
                this.collision(position, "pipe", cc.v2((size.width / 2) * this.dir, 0)) ) {
                    ...
                    this.turnDir();
            } else if ( this.collision(position, "trap", cc.v2( 0, -(size.height / 2 +
                    this.tiledMap.getTiledObject()._tileSize.height + 4 ))) ) {
                //判断移动后是否处于陷进层上方
                this.turnDir();
